...
impl<T: 'static + Clone + mlua::MaybeSend + mlua::MaybeSync> LuaTypeBinder<T> {
    ...
    pub fn register(self, lua: &Lua, type_table: &mlua::Table) -> Result<(), LuaBindError> {
        ...
        // Async dyn names join BEFORE the mangled-name scan below, so a
        // mangled monomorph colliding with an async dyn method is caught.
        #[cfg(all(feature = "generics", feature = "async", not(feature = "send")))]
        {
        ...
        #[cfg(feature = "generics")]
        {
            ...
            let generic_names = self
                ...
                .iter()
                .map(|(n, ..)| n)
                .chain(self.generic_mut_methods.iter().map(|(n, ..)| n));
            #[cfg(all(feature = "async", not(feature = "send")))]
            let generic_names = generic_names
                .chain(self.generic_async_methods.iter().map(|(n, ..)| n))
                .chain(self.generic_async_mut_methods.iter().map(|(n, ..)| n));
        ...
        // mangled-generic entries all share the namespace.
        {
            ...
            #[cfg(all(feature = "generics", feature = "async", not(feature = "send")))]
            let names = names.chain(self.dyn_async_associated.iter().map(|(n, _)| *n));
            for name in names {
            ...
            #[cfg(feature = "generics")]
            {
                ...
                let generic_names = self.generic_associated.iter().map(|(n, ..)| n);
                #[cfg(all(feature = "async", not(feature = "send")))]
                let generic_names =
                    generic_names.chain(self.generic_async_associated.iter().map(|(n, ..)| n));
                for name in generic_names {
                    if seen.contains(&name.as_str()) {
                        return Err(LuaBindError::DuplicateMethod { name: name.clone() });
                    }
        ...
        lua.register_userdata_type::<T>(move |reg| {
            ...
            // Async methods: the async block owns mlua's userdata guard
            // and the wrapper's future borrows it — zero-clone dispatch
            // (`&mut self` methods hold the MUTABLE guard, so mutation
            // writes back to the bound userdata in place). mlua's plain
            // async methods exist on every Lua version (unlike async
            // metamethods) but still require its `async` feature and, under
            // `send`, `Send` futures — those combos are rejected in
            // `method_async`.
            #[cfg(all(feature = "async", not(feature = "send")))]
                ...
                reg.add_async_method(
                    *name,
                    move |lua, this: mlua::UserDataRef<T>, args: mlua::MultiValue| {
                        let sv_args: mlua::Result<Vec<ScriptValue>> = args
                            .into_vec()
                            .iter()
                            .map(lua_to_script)
...
impl<T: 'static + Clone + mlua::MaybeSend + mlua::MaybeSync> TypeBinder<T> for LuaTypeBinder<T> {
    ...
    fn associated_generic(
        ...
        #[cfg(feature = "generics")]
        {
            self.generic_associated.push((
                mangle_generic_name(name, type_args),
                name,
                type_args,
                f,
            ));
    ...
    fn associated_generic_async(
        ...
        #[cfg(all(feature = "generics", feature = "async", not(feature = "send")))]
        {
            self.generic_async_associated.push((
                mangle_generic_name(name, type_args),
                name,
                type_args,
                f,
    ...
    fn method_generic(
        ...
        #[cfg(feature = "generics")]
        {
            self.generic_methods
                .push((mangle_generic_name(name, type_args), name, type_args, f));
            Ok(())
        }
    }
    ...
    fn method_generic_async(
        ...
        #[cfg(all(feature = "generics", feature = "async", not(feature = "send")))]
        {
            self.generic_async_methods.push((
                mangle_generic_name(name, type_args),
                name,
                type_args,
                f,
    ...
    fn method_generic_async_mut(
        ...
        #[cfg(all(feature = "generics", feature = "async", not(feature = "send")))]
        {
            self.generic_async_mut_methods.push((
                mangle_generic_name(name, type_args),
                name,
                type_args,
                f,
...
impl LuaFnBinder {
    ...
    pub fn apply(self, lua: &Lua, table: &mlua::Table) -> Result<(), LuaBindError> {
        ...
        #[cfg(feature = "generics")]
        {
            ...
            let generic_names = self.generic_functions.iter().map(|(n, ..)| n);
            #[cfg(all(feature = "async", not(feature = "send")))]
            let generic_names =
                generic_names.chain(self.generic_async_functions.iter().map(|(n, ..)| n));
            for name in generic_names {
                if seen.contains(&name.as_str()) {
                    return Err(LuaBindError::DuplicateMethod { name: name.clone() });
                }
...
impl FnBinder for LuaFnBinder {
    ...
    fn function_async(
        ...
        #[cfg(all(feature = "async", not(feature = "send")))]
        {
            #[cfg(feature = "generics")]
            if !type_args.is_empty() {
                self.generic_async_functions.push((
                    mangle_generic_name(name, type_args),
                    name,
                    type_args,
                    f,
