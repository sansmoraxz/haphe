...
use crate::types::{
    EnumDescriptor, PropertyDescriptor, StructDescriptor, TypeAliasDescriptor, TypeDescriptor,
    TypeId,
};
...
/// A foreign interface: host-supplied functions Rust calls out to.
///
/// Implemented by `#[script(foreign)]` on a trait, on the generated handle
/// type (not the trait itself, since traits cannot be named in `const`
/// descriptor positions).
#[diagnostic::on_unimplemented(
    message = "`{Self}` is not a foreign-interface handle",
    note = "entries under `foreign:` in `registry!` are the handle types generated by `#[script(foreign)]` on a trait"
)]
pub trait ScriptForeign {
    /// The interface's descriptor (erased: uniform across instantiations of
    /// a generic interface).
    const DESCRIPTOR: ForeignInterfaceDescriptor<'static>;
    /// Concrete type arguments of this handle's instantiation, in declaration
    /// order of the interface's generic parameters. Empty for non-generic
    /// interfaces. Backends that monomorphize use these to identify the
    /// instantiation a handle dispatches to.
    const TYPE_ARGS: &'static [TypeDescriptor<'static>] = &[];
}
